...
import { criarEquipamentoParaCliente } from "@/lib/brothers/clientes-data";
import { TecnicoShell } from "@/components/brothers/TecnicoShell";

export const Route = createFileRoute("/lista-clientes/$id/equipamentos/novo")({
  head: () => ({ meta: [{ title: "Novo equipamento — Brothers" }] }),
  component: NovoEquipamentoTecnicoPage,
});

function NovoEquipamentoTecnicoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const equipamentoId = await criarEquipamentoParaCliente({
        clienteId,
        tipo,
        ambiente,
        marca,
      ...
      navigate({ to: "/lista-clientes/$id/equipamentos/$equipId/finalizar", params: { id: clienteId, equipId: equipamentoId } });
    } catch (err: any) {
    ...
    <TecnicoShell>
      <div className="space-y-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Cadastre o equipamento em nome do cliente</p>
        </div>
